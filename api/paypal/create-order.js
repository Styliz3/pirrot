...
const PAYPAL_API = 'https://api-m.paypal.com'; // use sandbox URL for testing
...
export default async function handler(req, res) {
  try {
    ...
    for(const it of items){
      const row = cat[it.sku]; if(!row) return res.status(400).json({ error:`unknown_sku:${it.sku}` });
      const qty = Math.max(1, Number(it.qty||1));
      const unit = Number(row[currency]);
      total += unit * qty;
      purchase_units[0].items.push({
        name: row.name, sku: it.sku, quantity: String(qty),
